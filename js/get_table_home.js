...
var remark_app_set = [];
    ...
    row.innerHTML = `
      ...
      toggleRow.innerHTML = `
          ...
          <div id = "buttContain${id_set[count]}">
            ...
            <button type="button" class="btn btn-danger col col-xxl-1 pb-2 cancelButton float-end" data-bs-toggle="modal" data-bs-target="#cancelCheckModal" id = "cancelButt${id_set[count]}">
              Cancel
            </button>
            </div>
        </td>
      ...
      let cancelButtonCheck = setStatus.textContent;
      if (cancelButtonCheck.includes('Cancel') || cancelButtonCheck.includes('Approved') || cancelButtonCheck.includes('Denied') ){
        $(`#cancelButt${id_set[count]}`).remove();
      }
  }
  ...
  function makeNameCont(id,name,dept){
  ...
  $(document).on('click','.cancelButton', function(){
    const parent = this.id;
    var rowMinus = parent.replace(/\D/g, '');
    cancelModalID = parseInt(rowMinus,10);
    console.log(cancelModalID)
  })
  $(document).on('click', '.cancelModalButt', function(){
    var dataSend = {
      applicant_status: 'Cancelled',
      id_count: cancelModalID,
    };
    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        for(let i=0;i<immediate_set.length;i++){
          ...
          var carIdentifier = Array.isArray(arrangement_set[i]) && arrangement_set[i].includes(userget);
  ...
  function createTable(){
    ...
    for (let i = 0; i < prepare_set.length; i++){
      if (em_name_set[i].includes(name)==true){
        createTableRow(i);
      }else if(prepare_set[i] && prepare_set[i].includes(name)==true){
        createTableRow(i);
      }
    } 
  }
